...
class Hyb_o2oModuleWxapp extends WeModuleWxapp {
  ...
  private function api_notice_increment($url, $data){
    ...
    public function doPageBase()
    {
        ...
        $list['s_ttthumb'] = $_W['attachurl'].$list['s_ttthumb'];
    ...
    */
    ...
    //查询商家二级分类
    public function doPageShangjiastyleerji(){
        global $_W,$_GPC;
        $uniacid = $_W['uniacid'];
        $openid = $_REQUEST['openid'];
        ...
        $list = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_fuwu_type")." WHERE uniacid=:uniacid and xt_name=:xt_name and xt_parentid=0",array(":uniacid"=>$uniacid,":xt_name"=>$sj['s_type']));
        $info = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fuwu_type")." WHERE uniacid=:uniacid and xt_parentid=:xt_parentid",array(":uniacid"=>$uniacid,":xt_parentid"=>$list['xt_id']));
        return $this->result(0,"success",$info);
    }
      ...
      if (!empty($list)) {
        foreach ($list as &$value) {
            ...
            if (empty($fuwuid)) {
                ...
                $fuwuids = implode(",",$fuwuid);
                
                //查询评分
                $pingfencount = pdo_fetchall("SELECT sum(p_fenshu) as fenshu FROM ".tablename('hyb_o2o_fuwupingjia')."where uniacid=:uniacid and p_sid in($fuwuids) ",array(":uniacid"=>$uniacid));
                $pingfennum = pdo_fetchcolumn("SELECT count(*)  FROM ".tablename('hyb_o2o_fuwupingjia')."where uniacid=:uniacid and p_sid in($fuwuids) ",array(":uniacid"=>$uniacid));
                if(empty($pingfencount[0]['fenshu'])){
                    $pingfen = "0"; 
                }else{
                    $pingfen = $pingfencount[0]['fenshu']/$pingfennum;
                }
                $value['shangjiapingfen'] = $pingfen;
    ...
    {   
        ...
        if ($base['pt_show']=='1') {
            ...
            $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_shangjia")." where uniacid=:uniacid and ruzhu_endtime!=:ruzhu_endtime and ruzhu_endtime>'".$time."' and s_status=:s_status and s_tuijian=:s_tuijian and s_address like '%$city%' and pingtai!=1 order by s_ids desc ",array(":uniacid"=>$uniacid,":ruzhu_endtime"=>"已到期","s_status"=>"审核通过","s_tuijian"=>"是"));
        }
        ...
        if (!empty($list)) {
            foreach ($list as &$value) {
                ...
                //查询下单数
                $xiadancount = pdo_fetchcolumn("SELECT count(*) FROM ".tablename("hyb_o2o_orderfuwu")." WHERE uniacid=:uniacid and o_store=:o_store",array(":uniacid"=>$uniacid,":o_store"=>$value['s_id']));
                ...
                if (empty($fuwuid)) {
                    ...
                    $fuwuid = implode(",",$fuwuid);

                    $pinlunshu = pdo_fetchcolumn("SELECT count(*) FROM ".tablename("hyb_o2o_fuwupingjia")." WHERE uniacid=:uniacid and p_sid in($fuwuid)",array(":uniacid"=>$uniacid));
                    $value['pinlunshu'] = $pinlunshu;
                }
    ...
    public function doPageFuwuxq()
    {
        ...
        //查询服务分类
        $fwtype = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_fuwu_type")." where uniacid=:uniacid and xt_id=:xt_id and xt_parentid!=0",array(":uniacid"=>$uniacid,":xt_id"=>$list['x_type']));
        foreach ($yaungong as $key2 => $value2) {
            if (!in_array($fwtype['xt_name'], $yaungong[$key2]['y_jineng'])) {
                unset($yaungong[$key2]);
            }
        }
    ...
    //查询技师入驻时长
    public function doPageJsrzsj(){
        global $_W,$_GPC;
        $uniacid = $_W['uniacid'];
        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_jishiset")." where uniacid=:uniacid",array(":uniacid"=>$uniacid));
        if (!empty($list)) {
            foreach ($list as &$value) {
                $value['r_content'] = $value['r_time']."天/(¥".$value['r_money'].")";
            }
  ...
  public function doPagePay()
  {
    ...
    public function doPageSjrz()
    {
        ...
        if ($rz['pingtai']=="0") {
          ...
          $save = pdo_update("hyb_o2o_userinfo",array("u_shangjia"=>"待审核","u_yuangong"=>"0"),array("u_id"=>$user['u_id']));
  ...
  //商户累计提现
    ...
    public function doPagePaysjxf()
    {
          ...
          $rz_id = $_REQUEST['rz_id'];
          $ruzhu = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjiaruzhu")." WHERE uniacid=:uniacid and r_id=:r_id",array(":uniacid"=>$uniacid,":r_id"=>$rz_id));
          ...
          if ($shangjia['ruzhu_endtime']=="已到期") {
              ...
              $ruzhutime = $ruzhu['r_time'];
              if ($ruzhutime>1) {
                 $ruzhu_endtime = date('Y-m-d H:i:s', strtotime("$time +$ruzhutime day"));
              }else{
                $ruzhudate = $ruzhutime*24;
                $ruzhu_endtime = date('Y-m-d H:i:s', strtotime("$time +$ruzhudate hours"));
              }
    ...
    public function doPagePayjsxf()
    {
          ...
          $money = $_REQUEST['money'];
          $rz_id = $_REQUEST['rz_id'];
          $ruzhu = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_jishiset")." WHERE uniacid=:uniacid and r_id=:r_id",array(":uniacid"=>$uniacid,":r_id"=>$rz_id));
          //查询商户信息
          $yuangong = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_yuangong")." where uniacid=:uniacid and y_openid=:y_openid",array(":uniacid"=>$uniacid,":y_openid"=>$openid));
          if ($yuangong['y_endtime']=="已到期") {
              $ruzhutime = $ruzhu['r_time'];
              if ($ruzhutime>1) {
                 $ruzhu_endtime = date('Y-m-d H:i:s', strtotime("$time +$ruzhutime day"));
              }else{
              ...
              $data = array("y_endtime" => $ruzhu_endtime); 
          }else{
              $time = $yuangong['y_endtime'];
              $ruzhutime = $ruzhu['r_time'];
              if ($ruzhutime>1) {
                 $ruzhu_endtime = date('Y-m-d H:i:s', strtotime("$time +$ruzhutime day"));
              }else{
          ...
          pdo_update("hyb_o2o_yuangong",$data,array("y_id"=>$yuangong['y_id']));
          $key = pdo_fetch('SELECT * FROM ' . tablename('hyb_o2o_parment') . " where uniacid=:uniacid",array(":uniacid"=>$uniacid));
          $moban = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_tongzhi")." where uniacid=:uniacid",array(":uniacid"=>$uniacid));
          $APPID = $key['appid'];
          $SECRET = $key['appsecret'];
  ...
  public function doPageAddgoodspingjia()
  {
    ...
    public function doPageYuangongzhuce(){
      ...
      if ($_REQUEST['come']=='js') {
        ...
        $ruzhu = "1";
        $rz_id = $_REQUEST['rz_id'];
        $rzinfo = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_jishiset")." WHERE uniacid=:uniacid and r_id=:r_id",array(":uniacid"=>$uniacid,":r_id"=>$rz_id));
        $tzshijian = $rzinfo['r_time'];
       
        if ($typs=="发布") {
            $time = date("Y-m-d H:i:s",time());
            $ruzhu_endtime = date('Y-m-d H:i:s', strtotime("$time +$tzshijian day"));
        } 
      }
      ...
      $data = array(
        ...
        "y_rz"=>$ruzhu,
        "form_id"=>$_REQUEST['form_id'],
        "y_zgeimg"=>$_REQUEST['zgeimg'],
        "y_endtime"=>$ruzhu_endtime,
      );
      ...
      $save = pdo_update("hyb_o2o_userinfo",$datas,array("u_id"=>$user['u_id']));
        ...
        /*通知商家*/
        if ($_REQUEST['come']=='js') {
             $params["PhoneNumbers"] = $ptsj['s_telphone'];         //接收人手机号
        }else{
    ...
    public function doPageYuangongxq()
    {
        ...
        if (!empty($info)) {
          ...
          if (strpos($info['y_zgeimg'],"http")===false) {
            $info['y_zgeimg'] = $_W['attachurl'].$info['y_imgpath2'];
          }
   ...
   //查询商家全部员工
    public function doPageYuangonglist()
    {
      ...
      if ($_REQUEST['come']=="guanli") {
        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_yuangong")." WHERE uniacid=:uniacid and y_sjname=:y_sjname and y_styles!='待审核' order by y_time desc ",array(":uniacid"=>$uniacid,":y_sjname"=>$shangjia['s_id']));
      }
      if ($_REQUEST['come']=="zhipai") {
        if ($shangjia['pingtai']=='1') {
            $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_yuangong")." WHERE uniacid=:uniacid and y_sjname=:y_sjname and y_styles!='待审核' and y_jin=0 and y_rz=1",array(":uniacid"=>$uniacid,":y_sjname"=>$shangjia['s_id']));
        }else{
            $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_yuangong")." WHERE uniacid=:uniacid and y_sjname=:y_sjname and y_styles!='待审核' and y_jin=0",array(":uniacid"=>$uniacid,":y_sjname"=>$shangjia['s_id']));
        }
      ...
      if ($_REQUEST['come']=='zhipaidingdan') {
        ...
        if ($shangjia['pingtai']=='1') {
            $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_yuangong")." WHERE uniacid=:uniacid and y_sjname=:y_sjname and y_styles!='待审核' and y_jin=0 and y_rz=0",array(":uniacid"=>$uniacid,":y_sjname"=>$shangjia['s_id']));
        }else{
            $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_yuangong")." WHERE uniacid=:uniacid and y_sjname=:y_sjname and y_styles!='待审核' and y_jin=0",array(":uniacid"=>$uniacid,":y_sjname"=>$shangjia['s_id']));
        }
        ...
        /*员工匹配筛选*/
        //查询订单
        $order = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_orderfuwu")." WHERE uniacid=:uniacid and o_id=:o_id",array(":uniacid"=>$uniacid,":o_id"=>$_REQUEST['o_id']));
        //查询服务
        ...
        //查询服务分类
        $fuwutype = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_fuwu_type")." WHERE uniacid=:uniacid and xt_id=:xt_id and xt_parentid!=0",array(":uniacid"=>$uniacid,":xt_id"=>$fuwu['x_type']));

        if (!empty($list)) {
            foreach ($list as $key => $values) {
                $list[$key]['y_jineng'] = unserialize($list[$key]['y_jineng']);
                if (!in_array($fuwutype['xt_name'], $list[$key]['y_jineng'])) {
                   unset($list[$key]);
                }
            }
        }
    ...
    public function doPageYuangongshenhe()
    {
      ...
      $openid = $_REQUEST['openid'];
      $shangjia = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjia")." WHERE uniacid=:uniacid and s_u_openid=:s_u_openid",array(":uniacid"=>$uniacid,":s_u_openid"=>$openid));
      $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_yuangong")." as y left join ".tablename("hyb_o2o_userinfo")." as u on y.y_openid=u.openid WHERE y.uniacid=:uniacid and y.y_sjname=:y_sjname and y.y_styles=:y_styles order by y.y_time desc ",array(":uniacid"=>$uniacid,":y_sjname"=>$shangjia['s_id'],":y_styles"=>"待审核"));
      return $this->result(0,"success",$list);
    } 
        ...
        if ($xiangmu['x_huiyuanstatus']=="1") {
              if ($user['u_type']!="0" && $user['u_typeendtime']>date("Y-m-d H:i:s",time()) && $user['u_typeendtime']!="已到期") {
                  ...
                  if ($zhekou['h_zhekou']>1) {
                    ...
                    $huiyaunyouhui = $count_money-($zhekou['h_zhekou']*$count_money);
                    $count_money = $count_money*$zhekou['h_zhekou'];
                  }
    ...
    public function doPagePayfuzf()
    {
        ...
        if($pay_type=="定金支付"){
            if ($zhifu=="余额支付") {
                //判断余额
                if ($user['u_money'] < $order['o_ding_money']) {
                    return $this->result(1,"error",array("desc"=>"余额不足请充值"));
                }else{
        ...
        if ($pay_type=="上门估价") {
            ...
            if ($zhifu=="余额支付") {
                //判断余额
                if ($user['u_money'] < $order['o_shangmen_money'] ) {
                    return $this->result(1,"error",array("desc"=>"余额不足请充值"));
                }else{
        ...
        if ($zhifu=="余额支付") {
            if ($order['o_pay_type']=="全额付款") {
                ...
                if ($order['o_type']=='到店付款') {
                    ...
                    $c_money = $order['o_count_money']*$choucheng;  //平台抽成金额 

                    if ($order['o_fwry_type']=='0' && !empty($order['o_fwry'])) {  //代表是员工服务
                        if (empty($yg['y_choucheng'])) {
                            $ychoucheng = "0";
                        }else{
                            $ychoucheng = $yg['y_choucheng'];
                        }
                        $ygc_money = ($order['o_count_money']-$c_money)*$ychoucheng;
                        $y_money = $yg['y_money']+$ygc_money;
                        $symxy['uniacid'] = $uniacid;
                        $symxy['openid'] = $yg['y_openid'];
                        $symxy['orderid'] = $order['o_id'];
                        $symxy['ordermoney'] = $order['o_count_money'];
                        $symxy['money'] = $ygc_money;
                        $symxy['time'] = date("Y-m-d H:i:s",time());
                        $symxy['type'] = "员工";
                        $symxy['status'] = "订单金额不含上门费用";
                        pdo_insert("hyb_o2o_symx",$symxy);

                        $sjc_money = $order['o_count_money']-$c_money-$ygc_money;
                        $s_money = $sj['s_money']+$sjc_money; //抽成后金额
                        $symxs['uniacid'] = $uniacid;
                        $symxs['openid'] = $sj['s_u_openid'];
                        $symxs['orderid'] = $order['o_id'];
                        $symxs['ordermoney'] = $order['o_count_money'];
                        $symxs['money'] = $sjc_money;
                        $symxs['time'] = date("Y-m-d H:i:s",time());
                        $symxs['type'] = "商家";
                        $symxs['y_choucheng'] = $ygc_money;
                        $symxs['p_choucheng'] = $c_money;
                        $symxs['status'] = "订单金额不含上门费用";
                        pdo_insert("hyb_o2o_symx",$symxs);                        
                        pdo_update("hyb_o2o_yuangong",array("y_typs"=>"空闲中","y_money"=>$y_money),array("y_id"=>$yg['y_id']));
                    }else{
                        $sjc_money = $order['o_count_money']-$c_money;
                        $symxs['uniacid'] = $uniacid;
                        $symxs['openid'] = $sj['s_u_openid'];
                        $symxs['orderid'] = $order['o_id'];
                        $symxs['ordermoney'] = $order['o_count_money'];
                        ...
                        $symxs['status'] = "订单金额不含上门费用";
                        pdo_insert("hyb_o2o_symx",$symxs);
                        $s_money = $sj['s_money']+$order['o_count_money']-$c_money; //商家抽成后金额
              ...
              if ($order['o_pay_types']=="0") {
                ...
                //查询优惠券
                $yhq = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_useryouhuiquan")." where uniacid=:uniacid and openid=:openid and x_id=:x_id",array(":uniacid"=>$uniacid,":openid"=>$order['openid'],":x_id"=>$order['o_xid']));
                if ($user['u_money']<$order['o_sheng_money']) {
                    return $this->result(1,"error",array("desc"=>"余额不足请充值"));
                }
                pdo_update("hyb_o2o_useryouhuiquan",array("type"=>"已使用"),array("id"=>$yhq['id']));
                ...
                if ($order['o_fwry_type']=='0') {  //代表是员工服务
                    if (empty($yg['y_choucheng'])) {
                        $ychoucheng = "0";
                    }else{
                        $ychoucheng = $yg['y_choucheng'];
              ...
              pdo_update("hyb_o2o_userinfo",array("u_money"=>$u_money),array("u_id"=>$user['u_id']));
                ...
                if ($order['o_count_money']!='0'){
                    ...
                    //查询优惠券
                    $yhq = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_useryouhuiquan")." where uniacid=:uniacid and openid=:openid and x_id=:x_id",array(":uniacid"=>$uniacid,":openid"=>$order['openid'],":x_id"=>$order['o_xid']));
                    if ($user['u_money']<$money || $user['u_money']<$order['o_count_money']) {
                     return $this->result(1,"error",array("desc"=>"余额不足请充值"));
                    }
  ...
  public function doPageAddfwpingjia()
  {
      ...
      public function doPageOrderfuwulist()
      {
          ...
          if($currentTab =='0'){
            ...
            if ($typs=="yuangong" && $cur=="0") {        //员工到店服务全部订单
              $dingdanlist = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_orderfuwu")."as o left join ".tablename("hyb_o2o_fuwu")."as x on o.o_xid=x.x_id where o.uniacid=:uniacid and o.o_fwry=:o_fwry and o.o_xiangmu_xingshi=:o_xiangmu_xingshi and o.o_type!=:o_type and o.o_type!=:o_types and o.o_store=:o_store and o.yg=0 order by o_id desc",array(":uniacid"=>$uniacid,":o_fwry"=>$yuangong['y_name'],":o_xiangmu_xingshi"=>"到店服务",":o_type"=>"已取消",":o_types"=>"已退款",":o_store"=>$yuangong['y_sjname']));
          ...
          if ($currentTab == '2') {
            ...
            if($typs=="yuangong" && $cur=="0"){        //员工到店服务服务中订单
                $dingdanlist = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_orderfuwu")."as o left join ".tablename("hyb_o2o_fuwu")."as x on o.o_xid=x.x_id where o.uniacid=:uniacid and o.o_fwry=:o_fwry and o.o_fw_type=:o_fw_type and o.o_xiangmu_xingshi=:o_xiangmu_xingshi  and o.o_type!='已取消' and o.o_type!='已完成' and o.o_type!='已删除' and o.o_store=:o_store and o.yg=0 order by o_id desc",array(":uniacid"=>$uniacid,":o_fwry"=>$yuangong['y_name'],":o_fw_type"=>"1",":o_xiangmu_xingshi"=>"到店服务",":o_store"=>$yuangong['y_sjname']));             
                foreach ($dingdanlist as $key => $values) {
                  if ($values['o_pay_type']=="全额付款" && $values['o_type']=="未支付") {
                      unset($dingdanlist[$key]);
                  }
            ...
            foreach ($dingdanlist as &$value) {
                ...
                if ($value['o_fwry_type']=='0') {
                    ...
                    $value['o_fwry_tel'] = $xinxi['y_telphone'];
                        ...
                        $value['o_fwry_tel'] = $sj['s_telphone'];
                }
    ...
    //服务人员修改
      public function doPageOrderfuwurysave(){
        ...
        pdo_update("hyb_o2o_yuangong",array("y_typs"=>"服务中"),array("y_id"=>$fwyg));
            ...
            // var_dump($params);var_dump($content);
            /*通知员工*/
            ...
            // var_dump($paramss);var_dump($content);
      }
      ...
      public function doPageHexiao()
      {
          ...
          $ordertime = explode(" ",$order['o_yy_riqi']);
          //查询商家
          $shangjia = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjia")." where uniacid=:uniacid and s_id=:s_id and s_u_openid=:s_u_openid",array(":s_id"=>$order['o_store'],":s_u_openid"=>$openid,":uniacid"=>$uniacid));
          if (!empty($shangjia)) {
              //核销人员为商家本身
              $jie =  "匹配已找到";
              ...
              pdo_update("hyb_o2o_shangjia",array("s_money +="=>$order['o_shangmen_money']),array("s_id"=>$order['o_store']));
          }else{
              //查询商家员工
    ...
    public function doPageOrderFwwcs()
    {
        ...
        $order = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_orderfuwu")." where uniacid=:uniacid and o_id=:o_id",array(":uniacid"=>$uniacid,":o_id"=>$o_id));
        //查询服务员工
        $fwyuangong = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_yuangong")." where uniacid=:uniacid and y_name=:y_name and y_sjname=:y_sjname",array(":uniacid"=>$uniacid,":y_name"=>$order['o_fwry'],":y_sjname"=>$order['o_store']));
        $sj = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjia")." WHERE uniacid=:uniacid and s_id=:s_id",array(":uniacid"=>$uniacid,":s_id"=>$order['o_store']));
        $choushui = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_fuwu_type")." WHERE uniacid=:uniacid and xt_name=:xt_name",array(":uniacid"=>$uniacid,":xt_name"=>$sj['s_type']));
        if (empty($choushui)) {
            $choucheng = "0";
        ...
        if (empty($fwyuangong['y_choucheng'])) {
            $ychoucheng = "0";
        }else{
            $ychoucheng = $fwyuangong['y_choucheng'];
        }
        if ($order['o_pay_type']=="全额付款") {
            if ($order['o_type']=="已付款") {
                    ...
                    if ($order['o_fwry_type']=='0') {  //代表是员工服务
                        $ygc_money = ($order['o_count_money']-$c_money)*$ychoucheng;
                        $y_money = $fwyuangong['y_money']+$ygc_money;
                        $symxy['uniacid'] = $uniacid;
                        $symxy['openid'] = $fwyuangong['y_openid'];
                        $symxy['orderid'] = $order['o_id'];
                        $symxy['ordermoney'] = $order['o_count_money'];
                        $symxy['money'] = $ygc_money;
                        $symxy['time'] = date("Y-m-d H:i:s",time());
                        ...
                        $symxs['status'] = "订单金额不含上门费用";
                        pdo_insert("hyb_o2o_symx",$symxs);                        
                        pdo_update("hyb_o2o_yuangong",array("y_typs"=>"空闲中","y_money"=>$y_money),array("y_id"=>$fwyuangong['y_id']));
                    }else{
                        $sjc_money = $order['o_count_money']-$c_money;
                        $symxs['uniacid'] = $uniacid;
                        $symxs['openid'] = $sj['s_u_openid'];
        ...
        if ($order['o_pay_type']=='上门估价') {
            ...
            if ($order['o_type']=='未支付') {
                $sjc_money = $order['o_shangmen_money'];
                $s_money = $sj['s_money']+$sjc_money; //抽成后金额
                $symxs['uniacid'] = $uniacid;
                $symxs['openid'] = $sj['s_u_openid'];
                $symxs['orderid'] = $order['o_id'];
                $symxs['ordermoney'] = $order['o_shangmen_money'];
                $symxs['money'] = $sjc_money;
                $symxs['time'] = date("Y-m-d H:i:s",time());
                $symxs['type'] = "商家";
                $symxs['y_choucheng'] = "0";
    ...
    public function doPageOrderfwtype()
    {
        ...
        if ($order['o_pay_type']=="上门估价") {
            if ($order['o_count_money']=='0') {
                ...
                $sjc_money = $order['o_shangmen_money'];
                $symxs['uniacid'] = $uniacid;
                $symxs['openid'] = $shangjia['s_u_openid'];
                $symxs['orderid'] = $order['o_id'];
                $symxs['ordermoney'] = $order['o_shangmen_money'];
                $symxs['money'] = $sjc_money;
                $symxs['time'] = date("Y-m-d H:i:s",time());
                ...
                if ($order['o_fwry_type']=='0') {
                    ...
                    $sjc_money = $order['o_count_money']-$c_money-$ygc_money;
                    $s_money = $shangjia['s_money']+$sjc_money; //抽成后金额
                    $symxs['uniacid'] = $uniacid;
                    $symxs['openid'] = $shangjia['s_u_openid'];
                    $symxs['orderid'] = $order['o_id'];
                    $symxs['ordermoney'] = $order['o_count_money'];
                    ...
                    $symxs['status'] = "订单金额不含上门费用";
                    pdo_insert("hyb_o2o_symx",$symxs);
                    $s_money = $shangjia['s_money']+$order['o_count_money']-$c_money; //商家抽成后金额 
                }
    ...
    //下单商家自主服务开始服务
    public function doPageOrdersjfwks(){
        global $_W,$_GPC;
        $uniacid = $_W['uniacid'];
        $o_id = $_REQUEST['o_id'];
        $openid =  $_REQUEST['openid'];
        $shangjia = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjia")." WHERE uniacid=:uniacid and s_u_openid=:s_u_openid",array(":uniacid"=>$uniacid,":s_u_openid"=>$openid));
        $userinfo = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_userinfo")." WHERE uniacid=:uniacid and openid=:openid",array(":uniacid"=>$uniacid,":openid"=>$openid));
        $order = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_orderfuwu")." WHERE uniacid=:uniacid and o_id=:o_id",array(":uniacid"=>$uniacid,":o_id"=>$o_id));
        $ordertime = explode(" ", $order['o_yy_riqi']);
        if ($order['o_pay_type']!='上门估价') {
            pdo_update("hyb_o2o_orderfuwu",array("o_fw_type"=>"1","o_fwry_type"=>"1","o_fwry"=>$shangjia['s_name'],"o_fwry_id"=>$userinfo['u_id']),array("o_id"=>$o_id));          
        }else{
            $sjc_money = $order['o_shangmen_money'];
            $symxs['uniacid'] = $uniacid;
            $symxs['openid'] = $shangjia['s_u_openid'];
            ...
            pdo_insert("hyb_o2o_symx",$symxs);
            pdo_update("hyb_o2o_shangjia",array("s_money +="=>$order['o_shangmen_money']),array("s_id"=>$order['o_store']));
            pdo_update("hyb_o2o_orderfuwu",array("o_fw_type"=>"2","o_fwry_type"=>"1","o_fwry"=>$shangjia['s_name'],"o_fwry_id"=>$userinfo['u_id']),array("o_id"=>$o_id));   
        }
    ...
    public function doPageUserfadan()
    {
      ...
      if ($current=="0") {
        //查询全部
        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid  and fa_openid=:fa_openid and del=0 order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_openid"=>$openid));
      }
      if ($current=="1") {
        //查询待审核
      ...
      if ($current=="4") {
        //查询已完成
        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_openid=:fa_openid and fa_style=:fa_style and del=0 order by fa_time desc",array(":uniacid"=>$uniacid,":fa_openid"=>$openid,":fa_style"=>"已完成"));
      }
      if (!empty($list)) {
        foreach ($list as &$value) {
    ...
    //发单删除
    public function doPageUserfadandel(){
        global $_W,$_GPC;
        $uniacid = $_W['uniacid'];
        $fa_id = $_REQUEST['fa_id'];
        pdo_update("hyb_o2o_fadan",array("del"=>"1"),array("fa_id"=>$fa_id));
    }
  ...
  public function doPageAddpdpingjia()
  {
    ...
    public function doPageQddtlist()
    {
        ...
        if ($come=='shangjia') {
            $sj = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjia")." WHERE uniacid=:uniacid and s_u_openid=:s_u_openid",array(":uniacid"=>$uniacid,":s_u_openid"=>$openid));
            $s_address = explode("-",$sj['s_address']);
            if (!in_array($city, $s_address)) {
                $list = [];
            }else{
                if ($sj['pingtai']=="1") {
                    ...
                    if(!empty($fenlei)  && empty($quyu) && empty($jiage)) {
                      $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%') order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));  
                    }
                    if (!empty($quyu)  && empty($fenlei) && empty($jiage)) {
                      $sheng = $city."".$quyu;
                    ...
                    if (!empty($fenlei)  && !empty($quyu)  && empty($jiage)) {
                      $sheng = $city."".$quyu;
                      $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like  '%$sheng%' or fa_fwaddresss like '%$city%') ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));
                    }
                    if (!empty($fenlei)  && !empty($jiage) && empty($quyu)) {
                      if ($jiage=="由高到低") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like  '%$city%' or fa_fwaddresss like '%$city%') order by fa_fwmoney desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));
                      }
                      else if ($jiage == "由低到高") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like  '%$city%' or fa_fwaddresss like '%$city%') order by fa_fwmoney asc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));
                      }
                      else if ($jiage=="最新发布") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%') order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));
                      }
                    ...
                    if (!empty($quyu)  && !empty($jiage)  && empty($fenlei)) {
                      ...
                      if ($jiage=="由高到低") {
                        ...
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (a_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%') order by fa_time desc",array(":uniacid"=>$uniacid,":fa_style"=>"派单中"));
                      }
                    }
                    ...
                    if (!empty($fenlei)  && !empty($quyu)  && !empty($jiage) ) {
                      $sheng = $city."".$quyu;
                      if ($jiage=="由高到低") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%')  order by fa_fwmoney desc",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));
                      }else if ($jiage == "由低到高") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%') order by fa_fwmoney asc",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));
                      }else if ($jiage == "最新发布") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%') order by fa_time desc",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));
                      }
                    }
                }else{
                    if(empty($quyu)  && empty($fenlei) && empty($jiage)) {
                      $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%') and fa_fwstyle1=:fa_fwstyle1 order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                    }
                    if(!empty($fenlei)  && empty($quyu) && empty($jiage)) {
                      $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and fa_fwstyle2=:fa_fwstyle2 and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%') order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",":fa_fwstyle2"=>$fenlei));  
                    }
                    if (!empty($quyu)  && empty($fenlei) && empty($jiage)) {
                      $sheng = $city."".$quyu;
                      $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%') and fa_fwstyle1=:fa_fwstyle1 order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                    }
                    if (!empty($jiage)  && empty($fenlei) && empty($quyu)) {
                      if ($jiage=="由高到低") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%') and fa_fwstyle1=:fa_fwstyle1  order by fa_fwmoney desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                      }else if ($jiage=="由低到高") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%') and fa_fwstyle1=:fa_fwstyle1  order by fa_fwmoney asc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                      }else if ($jiage=="最新发布") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%') and fa_fwstyle1=:fa_fwstyle1  order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                      }
                    }
                    ...
                    if (!empty($quyu)  && !empty($jiage)  && empty($fenlei)) {
                      $sheng = $city."".$quyu;
                      if ($jiage=="由高到低") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%') and fa_fwstyle1=:fa_fwstyle1 order by fa_fwmoney desc",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                      }else if ($jiage == "由低到高") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%') and fa_fwstyle1=:fa_fwstyle1 order by fa_fwmoney asc",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                      }else if ($jiage=="最新发布") {
                        $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (a_fwaddress like '%$sheng%' or fa_fwaddresss like '%$sheng%') and fa_fwstyle1=:fa_fwstyle1 order by fa_time desc",array(":uniacid"=>$uniacid,":fa_style"=>"派单中",':fa_fwstyle1'=>$sj['s_type']));
                      }
                    }
            ...
            $js = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_yuangong")." WHERE uniacid=:uniacid and y_openid=:y_openid and y_rz=1",array(":uniacid"=>$uniacid,":y_openid"=>$openid));
            $jineng = unserialize($js['y_jineng']);
            if(empty($quyu)  && empty($fenlei) && empty($jiage)) {
                $list = pdo_fetchall("SELECT * FROM ".tablename("hyb_o2o_fadan")." WHERE uniacid=:uniacid and fa_style=:fa_style and (fa_fwaddress like '%$city%' or fa_fwaddresss like '%$city%')  order by fa_time desc ",array(":uniacid"=>$uniacid,":fa_style"=>"派单中"));
            }
            if(!empty($fenlei)  && empty($quyu) && empty($jiage)) {
            ...
            if (!empty($list)) {
                foreach ($list as $key=>$value) {
                    if (!in_array($value['fa_fwstyle2'],$jineng)) {
                        unset($list[$key]);
                    }
                    $value['fa_fwimgpath'] = unserialize($value['fa_fwimgpath']);
                }
                foreach ($list as &$values) {
                    $values['fa_fwimgpath'] = unserialize($values['fa_fwimgpath']);
                }
            }
        }
    ...
    //技师服务分类
    public function doPageJsstyleerji(){
        global $_W,$_GPC;
        $uniacid = $_W['uniacid'];
        $openid = $_REQUEST['openid'];
        $js = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_yuangong")." WHERE uniacid=:uniacid and y_openid=:y_openid and y_rz=1",array(":uniacid"=>$uniacid,":y_openid"=>$openid));
        $jineng = unserialize($js['y_jineng']);
        return $this->result(0,"success",$jineng);
    }
        ...
        if (!empty($list)) {
            foreach ($list as &$value) {
                ...
                if (!empty($value['q_pdname'])) {
                    ...
                    //查询接单者
                    $sj = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjia")." WHERE uniacid=:uniacid and s_u_openid=:s_u_openid",array(":uniacid"=>$uniacid,":s_u_openid"=>$value['q_openid']));
                    if (strpos($sj['s_thumb'],"http")===false) {
    ...
    //商家发单服务
    public function doPageQiangdanfwsj(){
        global $_W,$_GPC;
        $uniacid = $_W['uniacid'];
        $openid = $_REQUEST['openid'];
        $id = $_REQUEST['id'];
        ...
        $money = $sj['s_money']+$qiangdan['fa_fwshagneng'];  //增加上门费用
      ...
      public function doPageFenxiaoadd()
      {
          ...
          if (empty($fxinfo)) {
             ...
             if ($fxinfo['f_type']=='0') {
                 $datas = array("uniacid"=>$uniacid,"f_openid"=>$_REQUEST['openid'],"f_name"=>$_REQUEST['f_name'],"f_tel"=>$_GPC['f_tel'],"f_address"=>$_REQUEST['f_address'],"f_type"=>"1","f_style"=>"待审核");
             }else{
    ...
    public function doPageFenxiaoaddxj()
    {
        ...
        $data = array("uniacid"=>$uniacid,"f_openid"=>$openid,"f_name"=>$f_name,"f_parentid"=>$f_parentid,"f_style"=>"待审核","f_time"=>date("Y-m-d H:i:s",time()),"f_type"=>"0");
        if (!empty($parentid)) {      
            //查询openid
...
/*
    ...
    public function doPageOrderfwwcygdel(){
        global $_W,$_GPC;
        $uniacid = $_W['uniacid'];
        $id = $_REQUEST['o_id'];
        pdo_update("hyb_o2o_orderfuwu",array("yg"=>"1"),array("o_id"=>$id));
    }


    // //服务分类二级
    // public function doPageFuwutypeej()
    // {
    //     global $_W,$_GPC;
    //     $uniacid = $_W['uniacid'];
    //     $xt_id = $_REQUEST['xt_id'];
    ...
    //     //查询项目
    //     $xiangmu = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_fuwu")." where uniacid=:uniacid and x_id=:x_id",array(":uniacid"=>$uniacid,":x_id"=>$_REQUEST['x_id']));
    //     $xiangmu['x_guigecontent'] = unserialize($xiangmu['x_guigecontent']);
    //     $savess  = pdo_update("hyb_o2o_fuwu",array("x_xiaoliang +="=>"1"),array("x_id"=>$x_id));

    //     if ($_REQUEST['guige']=='1') {
    //         if ($xiangmu['x_guigecontent'][0]['guigexiang']=="SUV") {
    //             $guige = $xiangmu['x_guigecontent'][1]['guigexiang'];
    //         }else{
    //             $guige = $xiangmu['x_guigecontent'][0]['guigexiang'];
    //         }
    ...
    //         "openid"=>$_REQUEST['openid'],
    //         "ordersn"=>date("Ymd").str_pad(mt_rand(1, 99999), 5,'0',STR_PAD_LEFT),
    //         "o_name" =>$_REQUEST['kh_name'],
    //         "o_telphone"=>$_REQUEST['kh_phone'],
    //         "o_address"=>"",
    //         "o_xxaddress"=>"",
    //         "o_xid"=>$_REQUEST['x_id'],
    //         "o_xiangmu_name" => $xiangmu['x_name'],
    ...
    //         "o_xiangmu_money"=>$_REQUEST['xiangmu_money'],
    //         "o_xiangmu_xingshi"=>$xiangmu['x_xingshi'],
    //         "o_xiangmu_guige"=>$guige,
    //         "o_xiangmu_guigemoney"=>$_REQUEST['money'],
    //         "o_num"=>"1",
    //         "o_yy_riqi"=>"",
    //         "o_fwry"=>"",
    //         "o_ding_money"=>"0",
    //         "o_count_money"=>$_REQUEST['money'],
    //         "o_beizhu"=>"",
    //         "o_xdtime"=>date("Y-m-d H:i:s",time()),
    //         "o_pay_type"=>"全额付款",
    //         "o_type"=>"已付款",
    //         "o_store"=>$xiangmu['x_shangjia'],
    //         "o_huiyaunyouhui"=>"0",
    //         "o_youhuiquanyouhui"=>"0",
    //         "o_manjianyouhui"=>"0",
    //         "o_hexiaomashuzi"=>"0",
    //         "o_xpptype" =>$xiangmu['x_parenttype'],
    //     );
    ...
    //     $accessKeyId = $aliduanxin['accessKeyId'];
    //     $accessKeySecret = $aliduanxin['accessKeySecret'];
    //     $sj = pdo_fetch("SELECT * FROM ".tablename("hyb_o2o_shangjia")." where uniacid=:uniacid and s_id=:s_id",array(":uniacid"=>$uniacid,":s_id"=>$xiangmu['x_shangjia']));
