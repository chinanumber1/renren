...
class WeixinTx {
    ...
    function __construct($appid, $mch_id, $openid,$amount,$desc,$appkey,$re_user_name,$partner_trade_no) {
        $this->appid = $appid;
        $this->openid = $openid;
        $this->mch_id = $mch_id;
        $this->key = $appkey;
    ...
    {
        $return = $this->tixianpay();
        return $return;
    }
    private function tixianpay() {   
        ...
        $data['partner_trade_no']=$this->partner_trade_no;//.time();//这个是订单号。
    ...
    private function xmltoarray($xml) {
      ...
      curl_setopt($ch,CURLOPT_SSLCERT,getcwd().'/cert/hyb_o2o/apiclient_cert.pem'); //这个是证书的位置
      curl_setopt($ch,CURLOPT_SSLKEY,getcwd().'/cert/hyb_o2o/apiclient_key.pem'); //这个也是证书的位置
